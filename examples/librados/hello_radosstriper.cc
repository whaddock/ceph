...
int main(int argc, char* argv[])
{
  ...
  std::cerr << "Get Keyring: " << ret << std::endl;
  if( ret < 0)
  {
  ...
  std::cerr << "done writing: " << fname << std::endl << std::endl;

  // Read the object back in
  std::cerr << "Reading: " << fname << "\nas: "<< obj_name << std::endl;
  bufferlist *stripe = &bl;
  size_t stripe_len = bl.length();
  uint64_t offset = 0;
  std::cerr << "Input buffer length is " << stripe_len << std::endl;
  int code = rs->read(obj_name,stripe,stripe_len,offset);
  std::cerr << "Return code: " << code << std::endl;
  std::cerr << "done reading: " << fname << std::endl;
  std::cerr << "Output buffer length is " << bl.length() << std::endl;
