...
void readCompletionsCollectionThread() {
  ...
  while (!aio_done) { 
    ...
    while(waiting_on_stripe && !completions_empty
	  && count < (uint32_t)K) {
      completions_lock.lock(); // *** Get completions lock ***
      if (completions.empty()) 
	completions_empty = true;
...
void radosWriteThread() {
  ...
  uint32_t index = 0;
  uint32_t offset = 0;
  std::queue <Shard> shards_in_flight;
  ...
  while (!writing_done) {
     ...
     */
    while (completions.size() > (uint32_t)concurrentios) {
	...
	return;
      ...
      */
      if (!shards_in_flight.empty()) {
	Shard _shard = shards_in_flight.front();
	...
	*/
	...
	std::cerr THREAD_ID << "There should be a shard in here. Something is wrong."
			    << std::endl;
	std::cerr.flush();
      }
...
v         keeping up with this position. We will use it to append to the objects
    ...
    if (do_write) {
      ...
      shards_in_flight.push(shard); // So we can dereference the buffer
#ifdef TRACE
      output_lock.lock();
...
#ifdef TRACE
      output_lock.lock();
      std::cerr THREAD_ID << "Write thread sleeping now." << std::endl;
      std::cerr.flush();
      output_lock.unlock();
#endif
      std::this_thread::sleep_for(thread_sleep_duration);
  ...
  // Wait for the remaining writes to finish
...
#ifdef TRACE
      output_lock.lock();
      std::cerr THREAD_ID << "Starting do_read in radosReadThread." << std::endl;
      std::cerr.flush();
      output_lock.unlock();
#endif
...
#ifdef TRACE
      output_lock.lock();
      std::cerr THREAD_ID << "index: " << shard.get_hash() << " stripe read started in radosReadThread()"
			  << std::endl;
